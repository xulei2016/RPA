...
class QuestionnaireController extends BaseController
{
    ...
    public function index(Request $request){
        $this->log(__CLASS__, __FUNCTION__, $request, "查看 问卷 任务中心");
        return view('admin/rpa/Questionnaire/index');
    }

    //RPA add
    public function add(Request $request){
        $this->log(__CLASS__, __FUNCTION__, $request, "添加 问卷 任务");
    ...
    public function edit(Request $request){
        ...
        $this->log(__CLASS__, __FUNCTION__, $request, "修改 问卷 任务");
        return view('admin/rpa/Questionnaire/edit',['info'=>$info]);
    }

    //RPA add immed
    public function immedtasks(Request $request){
        $this->log(__CLASS__, __FUNCTION__, $request, "立即发布 问卷 页");
        return view('admin/rpa/Questionnaire/add_immed');
    }

    //RPA insert
    public function insert(Request $request){
        $data = $this->get_params($request, ['description','date','week','time','jsondata']);
        $data['name'] = 'sdxTestWirte';
        $data['week'] = isset($data['week']) ? implode(',',$data['week']) :'';
        $data['created_at'] = $this->getTime();
        $this->log(__CLASS__, __FUNCTION__, $request, "插入 问卷 信息");
        $this->immedtask();
    ...
    public function update(Request $request){
        ...
        $this->log(__CLASS__, __FUNCTION__, $request, "更新 问卷 信息");
        $this->immedtask();
        //查询rpa任务表tid
    ...
    public function delete(Request $request){
        $id = $request->id;
        $this->log(__CLASS__, __FUNCTION__, $request, "删除 问卷");
        $this->immedtask();
    ...
    public function deleteAll(Request $request){
        $ids = $request->ids;
        $this->log(__CLASS__, __FUNCTION__, $request, "批量删除 问卷 列表");
        $this->immedtask();
    ...
    public function immedtask(){
        $data = ['name'=>'taskdistribution','jsondata'=>'{"name":"sdxTestWirte"}'];
