...
class JJRVisController extends BaseController
{
    ...
    public function insertImmedtasks(Request $request){
        ...
        $task['jsondata'] = json_encode(['namelist' => isset($task['jsondata']) ? implode(',',$task['jsondata']) :'']);
    ...
    public function typeChange(Request $request){
        ...
        return $this->JJRVis->update(['status'=>1], $id);
    }
    ...
    public function JJRpagination(Request $request){
        ...
        $sort = ['id','asc'];
        $num = $selectInfo['num'];
        return $this->JJRVis->paginate($condition, $sort, $num);
    }
}
