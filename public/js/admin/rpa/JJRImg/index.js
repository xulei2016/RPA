...
$(function(){
        ...
        $("#container .inner-content .middle-layer .deletebatch").on('click', function(){
            ...
            if("" == ids){
	            ...
	            myConfirmModal("确定要批量删除资讯吗？",function(){
		            $.ajax({
		                url:"/admin/rpa_jjr_image/deleteAll",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function deleteOne(){
        ...
        myConfirmModal("确定要删除该资讯吗？",function(){
           $.ajax({
               url:"/admin/rpa_jjr_image/delete",
               type:"post",
               data:{"id":ids},
               dataType:"json",
               beforeSend:function(xhr){
    ...
    function render(pageIndex){
        $.ajax({
            type:'post',
            url:'/admin/rpa_jjr_image/rpa_list',
            ...
            success:function(json){
                if(json.code == 200){
                    ...
                    for(let i = 0; i < data.data.length;i++){
                        ...
                        let time = (row.time.length > 18) ? row.time.substring(0,16)+'···' : row.time ;
                        ...
                        +'<a class="btn btn-primary btn-sm edit" href="javascript:void(0);" data_id="'+id+'" url="/admin/rpa_rtc_collect/edit/'+id+'" onclick="operation($(this))">修改</a> '
